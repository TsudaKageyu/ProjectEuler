...
namespace
{
    const std::array<const char *, 1000> Hands = {
    ...
    {
        static const char *Numbers = "__23456789TJQKA";

        const char *p = ::strchr(Numbers, c);
        if (p)
            return static_cast<int>(p - Numbers);
        else
            return 0;
    }
    ...
    struct Hand
    {
        ...
        Hand(int h, int n1 = 0, int n2 = 0, int n3 = 0, int n4 = 0, int n5 = 0) :
            h(h),
            n({{ n1, n2, n3, n4, n5 }}) {}
    };
    ...
    bool operator<(const Hand &lhs, const Hand &rhs)
    {
        ...
        for (size_t i = 0; i < lhs.n.size(); ++i)
        {
            if (lhs.n[i] != rhs.n[i])
                return (lhs.n[i] < rhs.n[i]);
        }

        return false;
    }

    Hand GetHand(const char *str)
    {
        assert(::strlen(str) >= 14);

        // Parse and sort the cards.

        std::vector<Card> cs;

        for (size_t i = 0; i < 5; ++i)
            cs.emplace_back(str + i * 3);

        std::sort(cs.begin(), cs.end());

        const auto &c0 = cs.front();

        // Check some combinable hands.

        const bool flash = std::all_of(
            cs.begin() + 1, cs.end(), [&](const Card &c) { return (c.s == c0.s); });
        const bool straight
            = ((cs[1].n == cs[0].n + 1) || (cs[1].n == 2 && cs[0].n == 14))
            && (cs[2].n == cs[1].n + 1)
            && (cs[3].n == cs[2].n + 1)
            && (cs[4].n == cs[3].n + 1);
        ...
        for (auto it1 = cs.rbegin(); it1 != cs.rend(); )
        {
            int count = 1;
            for (auto it2 = it1 + 1; it2 != cs.rend(); ++it2)
            {
                if (it2->n != it1->n)
                    break;
            ...
            if (count == 4)
                four.push_back(it1->n);
            else if (count == 3)
                three.push_back(it1->n);
            else if (count == 2)
                two.push_back(it1->n);
            else if (count == 1)
                one.push_back(it1->n);
        ...
        if (four.size() == 1)
            return Hand(8, four[0], one[0]);

        // Full House

        if (three.size() == 1 && two.size() == 1)
            return Hand(7, three[0], two[0]);

        // Flash

        if (flash)
            return Hand(6, cs[4].n, cs[3].n, cs[2].n, cs[1].n, cs[0].n);
        ...
        if (three.size() == 1)
            return Hand(4, one[0], one[1]);

        // Two Pairs

        if (two.size() == 2)
            return Hand(3, two[0], two[1], one[0]);

        // One Pair

        if (two.size() == 1)
            return Hand(2, two[0], one[0], one[1], one[2]);

        // High Card

        if (one.size() == 5)
            return Hand(1, one[0], one[1], one[2], one[3], one[4]);
...
int64_t Problem54()
{
    ...
    {
        if (GetHand(hand + 15) < GetHand(hand))
            answer++;
    }
